...
export default class Conta {
  ...
  constructor(id: number, numero: string, cliente: Cliente, saldo: number, limite: number, idOperacaoAtual: number) {
    this._id = id;
    this._numero = numero;
    ...
    this._limite = limite;
    this._idOperacaoAtual = idOperacaoAtual;
  }

  public sacar(valor: number): Operacao {
    if (valor > 0) {
      this._saldo -= valor;
      console.log( `Saque na conta ${this._saldo.toFixed(2)}`);
